    ...
    methods
        ...
        function self = Logger(appname, options)
            ...
            if exist(self.filename, 'file') == 2
                % from version 2025b fopen('all') throws an error, this check ensure compatibility with both latest and older versions
                if feval(@(x)str2double(x(1:end-1)) > 2023, version("-release"))
                    fds = openedFiles;
                else
                    fds = fopen('all');
                end
